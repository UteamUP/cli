...
func TestRadioRoutesUseActiveTenantAndPublicGuids(t *testing.T) {
	...
	expected := map[string]string{"status": "environment", "policy": "policy", "channels": "history-channels", "recordings": "recordings", "get": "recordings/{recordingGuid}", "playback": "recordings/{recordingGuid}/playback"}
	for _, action := range domain.Actions {
		...
		for _, arg := range action.Args {
			if arg.Name != "recordingGuid" || arg.Type != "non-empty-uuid" {
