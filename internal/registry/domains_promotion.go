...
func init() {
	Register(&Domain{
		...
		Actions: []Action{
			...
			{
				Name:        "grant",
				Description: "Grant an existing promotion to a tenant (optionally bumping the plan)",
				ToolName:    "UteamupPromotionGrant",
				HTTPMethod:  "POST",
				RESTPath:    "by-guid/{guid}/grant",
				Args:        []ArgDef{{Name: "guid", Description: "Promotion GUID", Required: true, Type: "string"}},
				Flags: []FlagDef{
					{Name: "tenant-guid", Description: "Tenant GUID to grant the discount to (required)", Required: true, Type: "string"},
			...
			{
				Name:        "grant-adhoc",
				Description: "One-shot admin grant: create an AdminGrant offer and grant it to a tenant in one step",
